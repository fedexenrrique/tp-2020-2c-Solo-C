...
void cargar_config(void) {
	...
	puerto_escucha = config_get_int_value(config_restaurante, "PUERTO_ESCUCHA");
	ip_sindicato = config_get_string_value(config_restaurante, "IP_SINDICATO");
	puerto_sindicato = config_get_int_value(config_restaurante, "PUERTO_SINDICATO");
...
t_log * iniciar_logger(void){
	return log_create("restaurante.log", "restaurante", 1, LOG_LEVEL_INFO);
}

t_config* leer_config(void){
	return config_create("restaurante.config");
}
